...
static DEVICE_ATTR(state,
		   S_IRUGO | S_IWUSR | S_IWGRP, proximitysensor_state_show,
		   NULL);
static DEVICE_ATTR(adc, S_IRUGO | S_IWUSR | S_IWGRP, proximitysensor_adc_show,
		   NULL);
...
static DEVICE_ATTR(poll_delay,
		   S_IRUGO | S_IWUGO, light_poll_delay_show,
		   light_poll_delay_store);
...
static void gp2a_dev_destroy(void)
{
	...
	if (gp2a->hw_teardown) {
		...
		if (ret < 0)
			pr_warn("%s: Failed to teardown HW (%d)", __func__,
				ret);
	}
...
#if defined(CONFIG_GP2A_MODE_B)
	/* ASD : Select switch for analog sleep function
...
#if defined(CONFIG_GP2A_MODE_B)
	/* SSD: Software shutdown function
		(0:shutdown mode, 1:opteration mode) */
...
static void gp2a_dev_work_func(struct work_struct *work_prox)
{
	...
	if (!gp2a)
		...
		if (ret < 0)
			pr_warn
			    ("%s: Failed to get proximity value (%d), ignored",
			     __func__, ret);
		else {
...
static void gp2a_dev_light_work_func(struct work_struct *work)
{
	if (!gp2a)
		...
		if (gp2a->adc < 0)
			...
			/*
			 * 1350*x/1023 is the formula
			 *  to convert from RAW GPADC data
			...
			if (!gp2a->lux)
				gp2a->lux = 1;
			input_report_rel(gp2a->light_input, REL_MISC,
					 gp2a->lux);
...
static int gp2a_i2c_read(struct i2c_client *client, u8 reg, u8 * val,
			 unsigned int len)
{
	int err;
	u8 buf[2];
				 ...
				 struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",
		       gp2a->
		       current_operation & GP2A_LIGHT_SENSOR_POWER_ON ? 1 : 0);
}
				     ...
				     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",
		       gp2a->
		       current_operation & GP2A_PROXIMITY_POWER_ON ? 1 : 0);
}
...
static int __devexit gp2a_i2c_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&gp2a->light_input->dev.kobj,
			   &light_attribute_group);
	input_unregister_device(gp2a->light_input);

	sysfs_remove_group(&gp2a->prox_input->dev.kobj,
...
static int __init gp2a_module_init(void)
{
	...
	if (ret < 0)
		pr_err("%s: Failed to add i2c driver for GP2A (%d)", __func__,
		       ret);
