                                       ...
                                       const char *buf, size_t len)
{
	...
	dev_info(lcd->dev,"s6e63m0 lcd_sysfs_store_lcd_power_con\n");
...
static DEVICE_ATTR(lcd_power_con, 0644,
...
static int __devinit s6e63m0_mcde_panel_probe(struct mcde_display_device *ddev)
{
	...
	lcd->gamma_table_count = sizeof(gamma_table) / (MAX_GAMMA_LEVEL * sizeof(int));
        ...
        ret = device_create_file(&(lcd->ld->dev), &dev_attr_lcd_power_con);
        if (ret < 0)
                dev_err(&(ddev->dev), "failed to add lcd_power_con sysfs entries\n");
