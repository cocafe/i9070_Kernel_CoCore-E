...
int display_msg_colored(char* txtMsg, unsigned int fontColor, unsigned int bgColor, 
		unsigned char fontSize, unsigned char retainPrevMsg)
{
	int colHeight=3;
	static int rowHeight=0;
	...
	/* display debug message */
	lcd_draw_font(colHeight, rowHeight, fontColor, bgColor, strlen(txtMsg), (unsigned char*)txtMsg, fontSize);

	return 0;
}
EXPORT_SYMBOL(display_msg_colored);
